...
// ligação da pagina publicar eventos com o firebase
    ...
    return(
       ...
       <div className="col-12 mt-5"> 
            ...
            <form>
                ...
                <div className="form-group">
                    <label>Upload da Foto {props.match.params.id ? '(caso queira manter a foto atual, não clique aqui!)' : null} :</label>
                    <input onChange={(e) => setFotoNova(e.target.files[0]) } type="file" className="form-control"/>
                </div>

                <div className="row">
                {   
                    carregando > 0 ? <div className="spinner-border mx-auto" role="status"><span class="visually-hidden">Loading...</span></div>
                   : <button onClick={props.match.params.id ? atualizar : cadastrar} type="button" className="btn btn-lg btn-block mt-3 mb-5 btn-cadastro ml-2 mr-2">{props.match.params.id ? 'Atualizar Evento' : 'Publicar Evento'}</button>
                }
                </div>
            ...
            <div className="msg-login text-center mt-2">
                    {msgTipo === 'sucesso' && <span><strong>SUCESSO!</strong>Evento publicado com sucesso! &#9989;</span>}                                        
                    {msgTipo === 'erro' && <span><strong>ERRO! </strong>Não foi possivel publicar o evento, tente novamente! &#10060;</span>}
                </div>
